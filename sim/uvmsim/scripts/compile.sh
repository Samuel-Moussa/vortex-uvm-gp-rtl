...
if [[ $NO_COMPILE -eq 0 ]]; then
    ...
    if [[ "${ISACOV:-0}" == "1" ]]; then
        ...
        # Which extensions to build. RV32I comes from Imperas; RV32M is
        # GENERATED by isacov/gen/gen_ext_coverage.py from Imperas' own dvplan
        # CSV (the generator is proven by regenerating RV32I byte-for-byte) and
        ...
        vlog -sv $COMPILE_OPTS \
            ...
            +incdir+"$VORTEX_UVM_HOME/isacov/ext" \
            +incdir+"$ISACOV_ROOT/riscvISACOV/source" \
            +define+COVER_BASE_RV32I +define+COVER_LEVEL_BASIC $ISACOV_EXT_DEFS \
            -f isacov.flist \
